...
import com.group.core.extension.onAvoidDoubleClick
import java.util.*
...
abstract class RecyclerViewAdapter(var context: Context?, enableSelectedMode: Boolean) :
    ...
    fun updateModel(position: Int, model: Any, viewType: Int) {
        getListWrapperModels()!![position].apply {
            this.model = model
    ...
    fun updateModel(start: Int, end: Int, viewType: Int) {
        listWrapperModels.forEach { it.viewType = viewType }
//        notifyItemRangeChanged(start,end)
    ...
    fun unSelectedAllItem() {
        if (isInSelectedMode) {
            ...
            for (i in 0 until size) {
                listWrapperModels[i].isSelected = false
            }
            notifyItemRangeChanged(0, itemCount)
        }
    }
    ...
    fun <T> getItem(position: Int, classType: Class<T>): T? {
        return if (position != RecyclerView.NO_POSITION)
            classType.cast(listWrapperModels[position].model)
        else null
    }
    ...
    open inner class NormalViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        init {
            itemView.onAvoidDoubleClick {
                if (adapterPosition != -1) {
                    onItemClick(this@RecyclerViewAdapter, this, VIEW_TYPE_NORMAL, adapterPosition)
                }
            }
        }
    }
